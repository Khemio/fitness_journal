import Card from "../components/Card";
import WorkoutForm from "../components/WorkoutForm";
import { For, Show, createSignal } from "solid-js";
import { useJournalContext, pb, Workout } from "../context/JournalContext";
import { useBeforeLeave } from "@solidjs/router";

//TODO Fix copy

export default function FoodJournal() {

    useBeforeLeave((e) => {
        if (isEditable() && !e.defaultPrevented) {
            e.preventDefault();
        }
    });

    const {isAuth, foodCopy, setCopy, workouts, mutateWorkouts} = useJournalContext();

    const [isEditable, setEditable] = createSignal(false);
    const [addingFood, setAddition] = createSignal(false);


    function toggleEdit () {
        setEditable(!isEditable())
    }

    function toggleAdition () {
        setAddition(!addingFood())
    }

    //! Here

    function copyFood(day) {
        setCopy([]);
        day.foods_eaten.forEach(food => {
            setCopy([...foodCopy(), food]);
        })
    }

    function clearCopy() {
        setCopy([]);
        console.log(foodCopy());
    }

    async function deleteWorkout(workout: Workout) {
        workout.exercises.map(async exercise => await pb.collection('exercises_done').delete(exercise.id, {'$autoCancel': false}))
        
        await pb.collection('workout_journal').delete(workout.id);
        mutateWorkouts(workouts().filter(d => d.id !== workout.id));

    }

    return (
        <>
            <div class="w-11/12 mx-auto">
                <Show when={!addingFood()} fallback={<WorkoutForm toggle={toggleAdition}/>}>
                    <div class="flex justify-evenly">
                        <h2 class='font-bold text-center text-3xl text-blue-400 my-8 ml-auto'>Workout Journal</h2>
                        <Show when={foodCopy().length === 0} fallback={
                            <div class="ml-auto flex gap-7">
                                <span class="text-lg flex flex-col justify-center">{`${foodCopy().length}`} items copied</span>
                                <button class="text-xl ml-auto" onClick={clearCopy}>Clear</button>
                            </div>
                        } >
                            <button class="text-xl ml-auto" onClick={toggleEdit}>{`${isEditable() ? 'Cancel' : 'Delete'}`}</button>
                        </Show>
                    </div>
                    
                    <div class="grid grid-cols-4 gap-5">
                        <div class="p-3 rounded-md text-center flex bg-blue-300 text-white 
                            transition-all hover:scale-105 hover:bg-blue-400"
                            onClick={toggleAdition}>
                            <button class="m-auto text-center text-5xl">
                                +
                            </button>
                        </div>
                        <Show when={workouts() && isAuth()} fallback={<p>Loading...</p>}>
                            <For each={workouts()} >
                                {(workout): any => (
                                    
                                    <Card id={workout.id} editable={isEditable()} journal={"workout"}
                                    
                                    // onClick={(e) => {
                                    //     e.preventDefault();
                                    //     copyFood(workout);
                                    // }}
                                    >
                                            <div class="relative">                                        
                                                <h2 class="text-xl mx-auto"
                                                onClick={(e) => {
                                                    e.preventDefault();
                                                    copyFood(workout);
                                                }}>
                                                    {workout.date}
                                                </h2>
                                                <Show when={isEditable()}>
                                                    <button class="absolute right-0 top-0 text-red-600 pointer-events-auto"
                                                        onClick={(e) => {
                                                            // e.preventDefault();
                                                            deleteWorkout(workout);
                                                        }}>
                                                        &#10005;
                                                    </button>
                                                </Show>                                            
                                                <div class="flex flex-col grow justify-between gap-5">
                                                        <Show when={workout.exercises} fallback={<p>Loading...</p>}>
                                                            <For each={workout.exercises}>
                                                                {(exercise): any => (
                                                                    
                                                                    <p>{exercise.name} - {exercise.sets} - {exercise.reps}</p>
                                                                    
                                                                )}
                                                            
                                                            </For>
                                                        </Show>
                                                    {/* <p >Calories: {workout.calories} Protein: {workout.protein}</p> */}
                                                </div>
                                            </div>
                                        
                                    </Card>
                                )}
                            </For>
                        </Show> 
                    </div>
                </Show>
            </div>
        </>
    )
}